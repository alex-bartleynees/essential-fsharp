...
module AsyncResultDemo =
    ...
    let login username password : Async<Result<AuthToken, LoginError>> =
        asyncResult {
            ...
            do! user |> isPwdValid password |> Result.requireTrue InvalidPassword
            do! user |> authorize |> AsyncResult.mapError UnAuthorised
            return! user |> createAuthToken |> Result.mapError TokenError
        }
